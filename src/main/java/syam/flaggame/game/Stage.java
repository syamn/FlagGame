...
public class Stage implements IStage {
    ...
    public int rollbackChests(CommandSender sender) {
        ...
        for (Location loc : chests) {
            ...
            InventoryHolder fromContainer = null; // チェストでなければここで例外 修正予定 →
                                                  // 7/22修正済み
            try {
    ...
    public Map<FlagState, HashMap<FlagType, Integer>> checkFlag() {
        ...
        // 全フラッグを回す
        flag: for (Flag flag : flags.values()) {
    ...
    public Cuboid getBase(GameTeam team) {
        if (team == null || !baseMap.containsKey(team)) return null;
